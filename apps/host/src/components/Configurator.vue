<template>
  <div
    class="absolute top-[2.5rem] hidden right-0 left-1 w-[14rem] p-3 bg-white dark:bg-surface-800 rounded-md shadow border border-surface-200 dark:border-surface-700 flex-col justify-start items-start gap-3.5 inline-flex origin-top z-10"
  >
    <div class="flex-col justify-start items-start gap-2 inline-flex pr-4">
      <span class="text-black dark:text-surface-0 text-sm font-medium"
        >Primary Colors</span
      >
      <div
...
const updateColors = (type, colors) => {
  ...
  document.startViewTransition(() => applyTheme(type, colors));
};
